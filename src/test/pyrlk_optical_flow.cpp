//
//wus1: this is the orginal version from cv examples
//
#include <iostream>
#include <vector>
...
int pyrlk_optical_flow(int argc, char* argv[])
{
...
#else
    //string fname0 = "/home/maven/dataset/opticalFlow/eval-gray-allframes/Army/frame07.png";
    //string fname1 = "/home/maven/dataset/opticalFlow/eval-gray-allframes/Army/frame08.png";
    string fname0 = "/home/maven/dataset/opticalFlow/car-colored/img0001.png";
    string fname1 = "/home/maven/dataset/opticalFlow/car-colored/img0002.png";
    //string flow_type = "sparse";
    bool is_sparse = true;
    bool useGray = true; //cmd.has("gray");
    int winSize = 8;
    int maxLevel = 2;
    int iters = 10;
    int points = 100;
    ...
    if (is_sparse)
    {
        ...
        downloadFeaturePts(d_prevPts, prevPts);

        vector<Point2f> nextPts(d_nextPts.cols);
        downloadFeaturePts(d_nextPts, nextPts);

        vector<uchar> status(d_status.cols);
        downloadStatus(d_status, status);

        namedWindow("PyrLK [Sparse]", WINDOW_NORMAL);
        drawArrows(frame0, prevPts, nextPts, status, Scalar(255, 0, 0));

        imwrite("/home/maven/wus1/opt-flow-evaluation/build/bin/tmp.jpg", frame0);
